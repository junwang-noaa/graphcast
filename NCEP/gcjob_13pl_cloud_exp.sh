...
#SBATCH --job-name=gctest
#SBATCH --output=gc_13pl_output_test.txt
#SBATCH --error=gc_13pl_error_test.txt
...
source /contrib/Linlin.Cui/miniconda3/etc/profile.d/conda.sh
conda activate mlwp

# going to the model directory
cd /contrib/Linlin.Cui/operational/graphcast/NCEP/
...
python3 run_graphcast_exp.py -i source-gdas_date-"$curr_datetime"_res-0.25_levels-"$num_pressure_levels"_steps-2.nc -w /contrib/graphcast/NCEP -l "$forecast_length" -p "$num_pressure_levels" -u yes -k no
#python3 run_graphcast_exp.py -i source-gfs_date-"$curr_datetime"_res-0.25_levels-"$num_pressure_levels"_steps-2.nc -w /contrib/graphcast/NCEP -l "$forecast_length" -p "$num_pressure_levels" -u yes -k no
